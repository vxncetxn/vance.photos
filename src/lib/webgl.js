...
export class WebglInit {
  constructor(props) {
    ...
    collections.forEach((collection) => {
      this.imageData[collection.slug] = [
  ...
  preloadLQIPTextures() {
    ...
    Object.entries(this.imageData).forEach(([collection, images]) => {
      images.forEach((img, i) => {
        ...
        this.lqipTexturesAdded.push(
          texture.loaded.then(
            () => (this.imageData[collection][i].lqipTexture = texture)
          )
        );
      });
    });
  ...
  addCollection(name, images) {
    images.forEach((img, i) => {
      ...
      let program = new Program(this.gl, {
        ...
        fragment,
        uniforms: {
          uTexture: { value: this.imageData[name][i].lqipTexture },
  ...
  setPosition(scroll, cursor) {
    this.imageStore.forEach((o) => {
      o.posX =
        -scroll.current + o.left - this.width / 2 + o.width / 2 - o.extraScroll;
